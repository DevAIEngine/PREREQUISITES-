...
export const SeniorFriendlyGeminiUI: React.FC<{ userId: string }> = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    const culturalTone = translations[language as keyof typeof translations]?.tone || translations["English"].tone;
  ...
  const handleCallToggle = () => {
    ...
    if (!isCalling) {
        setMessages([{ timestamp: new Date().toISOString(), role: "gemini", content: translations[language as keyof typeof translations]?.greeting || translations["English"].greeting }]);
    } else {
        setMessages([]);
    }
  };
                  ...
                  <div style={{ marginTop: "40px", display: "flex", gap: "20px" }}>
                      {!isCalling ? (
                          <button onClick={handleCallToggle} style={{ fontSize: "36px", padding: "20px 60px", backgroundColor: "#22c55e", color: "white", border: "none", borderRadius: "100px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 0 30px rgba(34, 197, 94, 0.6)", transition: "transform 0.2s" }}>
                              <span aria-hidden="true">📞</span> {translations[language as keyof typeof translations]?.btnCall || translations["English"].btnCall}
                          </button>
                      ) : (
                          <button onClick={handleCallToggle} style={{ fontSize: "36px", padding: "20px 60px", backgroundColor: "#ef4444", color: "white", border: "none", borderRadius: "100px", fontWeight: "bold", cursor: "pointer", boxShadow: "0 0 30px rgba(239, 68, 68, 0.6)" }}>
                              <span aria-hidden="true">☎️</span> {translations[language as keyof typeof translations]?.btnEnd || translations["English"].btnEnd}
                          </button>
                      )}
                  </div>
              </div>
