...
*   **Offline "Parallel L5" Processing (WebGPU):** GUCE explicitly offloads "Ken Burns" panning and simple 2D motion processing directly to the user's local hardware via the browser's WebGPU (`navigator.gpu`). The system refuses to process simple 2D motion in the cloud.
