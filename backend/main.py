...
def get_api_key(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if not expected_api_key:
        logger.warning("GUCE_API_KEY environment variable not set. Rejecting requests.")
        raise HTTPException(status_code=401, detail="Internal server configuration error")
...
async def start_capture_stream(user_id: str, background_tasks: BackgroundTasks, api_key: str = Depends(get_api_key)):
    """
