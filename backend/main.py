...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    if not expected_api_key:
        logger.error("GUCE_API_KEY environment variable not configured")
        raise HTTPException(status_code=500, detail="Internal Server Error: Missing Configuration")

    # Use hmac.compare_digest to prevent timing attacks
    if not hmac.compare_digest(credentials.credentials.encode("utf-8"), expected_api_key.encode("utf-8")):
...
async def start_capture_stream(user_id: str, background_tasks: BackgroundTasks, api_key: str = Depends(get_current_user)):
    """
